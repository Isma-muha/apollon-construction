...
const Home: NextPage = () => {
  ...
  const methodeSteps = [
    { num:'01', icon:'📞',
      title: locale==='nl'?'Gratis bezoek':locale==='en'?'Free site visit':'Visite gratuite',
      desc: locale==='nl'?'We komen ter plaatse om de werf te evalueren, de maten te nemen en uw behoeften te begrijpen.':locale==='en'?'We visit your site to assess the works, take measurements and understand your needs.':'On se déplace chez vous pour évaluer le chantier, prendre les mesures et comprendre vos besoins.' },
    { num:'02', icon:'📋',
      title: locale==='nl'?'Gedetailleerde offerte binnen 48u':locale==='en'?'Detailed quote within 48h':'Devis détaillé sous 48h',
      desc: locale==='nl'?'Een duidelijke offerte, post per post, zonder verborgen kosten. U weet precies wat u betaalt.':locale==='en'?'A clear quote, line by line, no hidden costs. You know exactly what you pay.':'Un devis clair, poste par poste, sans frais cachés. Vous savez exactement ce que vous payez.' },
    { num:'03', icon:'🔨',
      title: locale==='nl'?'Verzorgde uitvoering':locale==='en'?'Quality execution':'Exécution soignée',
      desc: locale==='nl'?'Ons eigen team voert de werken uit. Eén aanspreekpunt houdt u op de hoogte bij elke stap.':locale==='en'?'Our own team carries out the works. One contact keeps you informed at every step.':'Notre équipe propre réalise les travaux. Un interlocuteur unique vous tient informé à chaque étape.' },
    { num:'04', icon:'✅',
      title: locale==='nl'?'Oplevering & garantie':locale==='en'?'Handover & guarantee':'Réception & garantie',
      desc: locale==='nl'?'Gezamenlijke eindkeuring. Werf opgeruimd, tienjarige garantie, reactieve after-sales service.':locale==='en'?'Final inspection together. Site cleaned, decennial guarantee, responsive after-sales service.':'Inspection finale ensemble. Chantier nettoyé, garantie décennale, service après-vente réactif.' },
  ]

  const pourquoiItems = [
    { title: locale==='nl'?'Tweetalig team FR/NL':locale==='en'?'Bilingual team FR/NL':'Équipe bilingue FR/NL',
      desc: locale==='nl'?'Twee vaste contactpersonen om u in uw taal te bedienen.':locale==='en'?'Two dedicated contacts to serve you in your language.':'Deux interlocuteurs dédiés pour vous servir dans votre langue.' },
    { title: locale==='nl'?'Reactiviteit':locale==='en'?'Reactivity':'Réactivité',
      desc: locale==='nl'?'Antwoord dezelfde dag, offerte binnen 48u.':locale==='en'?'Same-day response, quote within 48h.':'Réponse le jour même, devis sous 48h.' },
    { title: locale==='nl'?'Totale transparantie':locale==='en'?'Total transparency':'Transparence totale',
      desc: locale==='nl'?'Gedetailleerde offerte post per post. Geen verrassingen op de eindrekening.':locale==='en'?'Detailed quote line by line. No surprises on the final invoice.':'Devis détaillé poste par poste. Pas de surprise à la facture finale.' },
    { title: locale==='nl'?'Verzekering & conformiteit':locale==='en'?'Insurance & compliance':'Assurance & conformité',
      desc: locale==='nl'?'BA beroepsaansprakelijkheid, AREI conformiteit, markttoelating in orde.':locale==='en'?'Professional liability, RGIE compliance, market access in order.':'RC professionnelle, conformité RGIE, accès au marché en règle.' },
  ]

  const reviews = [
    { name:'Mahmood Hussein', date: locale==='nl'?'4 maanden geleden':locale==='en'?'4 months ago':'il y a 4 mois',
      text: locale==='nl'?'Uitstekende service van Apollon Construction. Verzorgd werk, punctueel en zeer professioneel van begin tot einde. Naleving van de termijnen en onberispelijk resultaat. Ik beveel sterk aan, dank aan Andi voor zijn communicatie!':locale==='en'?'Excellent service from Apollon Construction. Careful work, punctual and very professional team. Deadlines respected and impeccable result. Highly recommend, thanks to Andi for his communication!':'Excellent service de la part d\'Apollon Construction. Travail soigné, équipe ponctuelle et très professionnelle du début à la fin. Respect des délais et résultat impeccable. Je recommande vivement !',
      badge:'Local Guide' },
    { name:'E.', date: locale==='nl'?'een jaar geleden':locale==='en'?'1 year ago':'il y a un an',
      text: locale==='nl'?'Top service! Snel en efficiënt. Ik had een elektrisch probleem en ze kwamen binnen 24u en losten alles op. Ik beveel sterk aan!':locale==='en'?'Top service! Fast and efficient. Electrical issue solved in under 24h. Highly recommend!':'Top service ! Rapide et efficace. Panne électrique résolue en moins de 24h. Je recommande fortement !',
      badge:'Local Guide' },
    { name:'Leona Muhaxheri', date: locale==='nl'?'5 maanden geleden':locale==='en'?'5 months ago':'il y a 5 mois',
      text: locale==='nl'?'Zeer tevreden over hun werk en hun professionalisme. Dank u voor uw werk.':locale==='en'?'Very satisfied with their work and professionalism. Thank you.':'Très satisfaite de leurs travaux et de leur professionnalisme. Merci pour votre travail.',
      badge:'' },
  ]

  const batibouw_desc = locale==='nl'?'Partner van de referentiebeurs voor bouw en renovatie in België. Een gage van ernst en engagement in de sector.':locale==='en'?'Partner of the reference trade fair for construction and renovation in Belgium. A mark of seriousness and commitment in the sector.':'Partenaire du salon de référence de la construction en Belgique. Un gage de sérieux et d\'engagement dans le secteur.'
  const trustup_desc = locale==='nl'?'Geverifieerde aannemer op TrustUp: financiële soliditeit, fiscale en sociale situatie gecontroleerd door het platform.':locale==='en'?'Verified contractor on TrustUp: financial solidity, tax and social situation checked by the platform.':'Entrepreneur vérifié sur TrustUp : solidité financière, situation fiscale et sociale contrôlées par la plateforme.'
  ...
  return (
    <Layout>
      ...
      <section className="bg-green py-12 px-[5%]">
        <dl className="grid grid-cols-2 md:grid-cols-3 gap-8 max-w-2xl mx-auto">
          {[
            { num:'48h', label: locale==='nl'?'Offerte termijn':locale==='en'?'Quote turnaround':'Délai de devis' },
            { num:'0€', label: locale==='nl'?'Offerte & verplaatsing':locale==='en'?'Quote & site visit':'Devis & déplacement' },
            { num:'100%', label: locale==='nl'?'Eigen team — geen onderaanneming':locale==='en'?'Own team — no hidden subcontracting':'Équipe propre — pas de sous-traitance cachée' },
          ].map(({ num, label }) => (
      ...
      <section className="py-20 px-[5%] bg-cream-2">
        <div className="text-center mb-14">
          <p className="flex items-center justify-center gap-2 text-xs font-medium text-clay tracking-widest uppercase mb-3">
            <span className="w-6 h-px bg-clay" aria-hidden="true" />
            {locale==='nl'?'Onze methode':locale==='en'?'Our method':'Notre méthode'}
            <span className="w-6 h-px bg-clay" aria-hidden="true" />
          </p>
          <h2 className="font-serif text-4xl font-normal text-ink">
            {locale==='nl'?'Van eerste contact tot oplevering':locale==='en'?'From first call to project handover':'Du premier appel à la réception des travaux'}
          </h2>
        </div>
        <ul className="grid md:grid-cols-4 gap-6" role="list">
          {methodeSteps.map(({ num, icon, title, desc }) => (
      ...
      <section className="py-20 px-[5%] bg-white">
        <div className="grid md:grid-cols-2 gap-16 items-center">
          ...
          <div className="bg-ink rounded-xl p-10 text-white">
            <h3 className="font-serif text-2xl font-normal text-white mb-5">
              {locale==='nl'?'"We beloven niet de grootste te zijn. We beloven de meest serieuze te zijn."':locale==='en'?'"We don\'t promise to be the biggest. We promise to be the most serious."':'"On ne promet pas d\'être les plus gros. On promet d\'être les plus sérieux."'}
            </h3>
            <p className="text-white/65 font-light leading-relaxed mb-8">
              {locale==='nl'?'Elke werf telt. We behandelen de uwe alsof het de onze was — want onze reputatie hangt er van af.':locale==='en'?'Every project counts. We treat yours as if it were ours — because our reputation depends on it.':'Chaque chantier compte. On traite le vôtre comme si c\'était le nôtre — parce que notre réputation en dépend.'}
            </p>
            <Link href="/contact" className="inline-block bg-green text-white px-8 py-3.5 rounded text-sm font-medium hover:bg-green-mid transition-colors">
              {locale==='nl'?'Gratis offerte aanvragen':locale==='en'?'Request a free quote':'Demander un devis gratuit'}
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-[5%] bg-cream-2">
        <div className="text-center mb-10">
          <p className="flex items-center justify-center gap-2 text-xs font-medium text-clay tracking-widest uppercase mb-3">
            <span className="w-6 h-px bg-clay" aria-hidden="true" />
            {locale==='nl'?'Onze certificeringen':locale==='en'?'Our certifications':'Nos certifications'}
            <span className="w-6 h-px bg-clay" aria-hidden="true" />
          </p>
          <h2 className="font-serif text-3xl font-normal text-ink">{locale==='nl'?'Erkend en gecertificeerd':locale==='en'?'Recognised and certified':'Reconnus et certifiés'}</h2>
        </div>
        <div className="grid md:grid-cols-2 gap-6 max-w-3xl mx-auto">
          <div className="bg-white border-2 border-green/20 rounded-xl p-8 text-center">
            ...
            <h3 className="font-serif text-xl font-normal text-ink mb-3">{locale==='nl'?'Partner Batibouw+':locale==='en'?'Batibouw+ partner':'Partenaire Batibouw+'}</h3>
            <p className="text-muted text-sm font-light leading-relaxed">{batibouw_desc}</p>
            <div className="mt-4 inline-flex items-center gap-1 text-xs text-green font-medium">
              {locale==='nl'?'✓ Officiële partner':locale==='en'?'✓ Official partner':'✓ Partenaire officiel'}
            </div>
          </div>
          <div className="bg-white border-2 border-yellow-400/30 rounded-xl p-8 text-center">
            ...
            <h3 className="font-serif text-xl font-normal text-ink mb-3">{locale==='nl'?'Geverifieerd TrustUp Pro':locale==='en'?'TrustUp Pro verified':'Certifié TrustUp Pro'}</h3>
            <p className="text-muted text-sm font-light leading-relaxed">{trustup_desc}</p>
            <div className="mt-4 inline-flex items-center gap-1 text-xs text-green font-medium">
              {locale==='nl'?'✓ Geverifieerd bedrijf':locale==='en'?'✓ Verified company':'✓ Entreprise vérifiée'}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-[5%] bg-white">
        <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
          <div>
            ...
            <h2 className="font-serif text-4xl font-normal text-ink">
              {locale==='nl'?'Wat onze klanten zeggen':locale==='en'?'What our clients say':'Ce que disent nos clients'}
            </h2>
          </div>
          <a href="https://share.google/aO4CXjZ8n3OlEY00c" target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-3 bg-white border border-cream-3 rounded-xl px-5 py-3 hover:border-green transition-colors shadow-sm">
            <div>
        ...
        <ul className="grid md:grid-cols-3 gap-5" role="list">
          {reviews.map(({ name, date, text, badge }) => (
            <li key={name} className="bg-cream-2 rounded-xl p-7 border border-cream-3 flex flex-col">
      ...
      {/* ZONES */}
      <section className="bg-cream py-8 px-[5%]">
              ...
              {[
                ...
                <div key={label} className="flex items-start gap-3">
                  ...
                  <div className="text-sm">
                    ...
                    {href ? <a href={href} target={href.startsWith('http') ? '_blank' : undefined} rel="noopener noreferrer" className="text-green hover:underline font-light">{value}</a> : <span className="text-muted font-light">{value}</span>}
                  </div>
                </div>
              ))}
            </address>
